...
class CanvasManager {
  ...
  addCtxTransformTracking() {
    ...
    _ctx.scale = function (sx, sy) {
      _ctx._custom_transform_matrix = _ctx._custom_transform_matrix.scaleNonUniform(
        sx,
        sy
      );
      checker();
    ...
    _ctx.translate = function (sx, sy) {
      _ctx._custom_transform_matrix = _ctx._custom_transform_matrix.translate(
        sx,
        sy
      );
      checker();
    ...
    _ctx.rotate = function (r) {
      _ctx._custom_transform_matrix = _ctx._custom_transform_matrix.rotate(
        (r * 180.0) / Math.PI
      );
      checker();
    ...
    _ctx.transform = function (a, b, c, d, e, f) {
      ...
      m2.e = e;
      m2.f = f;
      _ctx._custom_transform_matrix = _ctx._custom_transform_matrix.multiply(
        m2
      );
      checker();
  ...
  translate(x, y) {
    ...
    this.user_transform = this.user_transform.translate(
      x / this.user_transform.a,
      y / this.user_transform.d
    );
  }
  ...
  translate_element(
    ...
    if (parent_element) {
      ...
      const parent_left_border = parent_element.x - parent_element.width / 2;
      const parent_rigth_border = parent_left_border + parent_element.width;
      const parent_top_border = parent_element.y - parent_element.height / 2;
      ...
      if (el instanceof Edge) {
        ...
        } else if (target_x >= max_x || new_mousepos.x >= parent_rigth_border) {
  ...
  mapPixelToCoordsX(xpos) {
    return this.svgPoint(xpos, 0).matrixTransform(this.user_transform.inverse())
      .x;
  }

  mapPixelToCoordsY(ypos) {
    return this.svgPoint(0, ypos).matrixTransform(this.user_transform.inverse())
      .y;
  }
  ...
  draw(now = null) {
    ...
    if (!now || !this.prev_time) dt = null;
    if (now) this.prev_time = now;
    ...
    if (this.request_scale && this.contention == 1) {
      ...
      this.request_scale = false;
    } else this.applyUserTransform();

    this.renderer.draw(dt);
    this.contention -= 1;

    if (this.animating) {
      if (!this.animate_target) this.animating = false;
      this.draw_async();
    }
  }
...
/**
 * @param {CustomCanvasRenderingContext2D} ctx
 * @param {SDFGNode} sdfg_state
 * @param {SDFGRoot} sdfg
 * @param {any} sdfg_list
 * @param {{ [x: string]: any; }} state_parent_list
 */
...
class SDFGRenderer {
  ...
  update_toggle_buttons() {
    if (this.mouse_mode == "move") {
      ...
      this.movemode_btn.innerHTML = '<i class="material-icons">open_with</i>';
      this.movemode_btn.title = "Enter object moving mode";
    }
    if (this.mouse_mode == "select") {
      this.selectmode_btn.innerHTML = '<i class="material-icons">done</i>';
  ...
  init_elements(user_transform) {
    ...
    move_mode_btn.onclick = () => {
      if (this.mouse_mode == "move") this.mouse_mode = "pan";
      else this.mouse_mode = "move";
    ...
    box_select_btn.onclick = () => {
      if (this.mouse_mode == "select") this.mouse_mode = "pan";
      else this.mouse_mode = "select";
      this.update_toggle_buttons();
    };
  ...
  set_mouse_handlers() {
    ...
    let comp_x = (event) =>
      this.canvas_manager.mapPixelToCoordsX(event.clientX - br().left);
    let comp_y = (event) =>
      this.canvas_manager.mapPixelToCoordsY(event.clientY - br().top);
  ...
  draw(dt) {
    ...
    if (this.box_select_rect) {
      ...
      this.ctx.rect(
        this.box_select_rect.x_start,
        this.box_select_rect.y_start,
  ...
  visible_elements() {
    ...
    this.do_for_intersected_elements(curx, cury, curw, curh, (type, e, obj) => {
      ...
      let el_type = "other";
      if (type === "nodes") el_type = "node";
      else if (type === "states") el_type = "state";
      else if (type === "edges") el_type = "edge";
      else if (type === "isedges") el_type = "isedge";
      else if (type === "connectors") el_type = "connector";
      elements.push({
        type: el_type,
  ...
  for_all_elements(x, y, w, h, func) {
    // Traverse nested SDFGs recursively
    /**
       * @param {dagre.graphlib.Graph<SDFGNode>} g
       * @param {any} sdfg_name
       */
    function traverse_recursive(g, sdfg_name) {
      g.nodes().forEach((state_id) => {
        ...
        ng.nodes().forEach((node_id) => {
          ...
          func(
            "nodes",
            { sdfg: sdfg_name, state: state_id, id: node_id, graph: ng },
            node,
        ...
        ng.edges().forEach((edge_id) => {
          ...
          func(
            "edges",
            { sdfg: sdfg_name, state: state_id, id: edge.id, graph: ng },
            edge,
  ...
  find_elements_under_cursor(mouse_pos_x, mouse_pos_y) {
    ...
    const total_elements =
      clicked_states.length +
      clicked_nodes.length +
      clicked_edges.length +
      clicked_interstate_edges.length +
      clicked_connectors.length;
    let foreground_elem = null,
      foreground_surface = -1;
  ...
  on_mouse_event(event, comp_x_func, comp_y_func, evtype = "other") {
    ...
    if (evtype === "mousedown" || evtype === "touchstart") {
      ...
      if (event.touches.length == 0) this.drag_start = null;
      else this.drag_start = event;
    ...
    if (total_elements > 0) {
      if (this.mouse_mode == "move" && this.drag_start)
        this.canvas.style.cursor = "grabbing";
      else if (this.mouse_mode == "move") this.canvas.style.cursor = "grab";
    ...
    this.for_all_elements(
      ...
      (type, e, obj, intersected) => {
        if (
          highlighted.indexOf(obj) == -1 &&
          (obj.stroke_color === "green" || obj.stroke_color === "orange")
        )
    ...
    if (evtype === "dblclick") {
      ...
      if (foreground_elem instanceof State)
        ...
        if (sdfg_elem.type.endsWith("Exit"))
          sdfg_elem =
            sdfg.nodes[foreground_elem.parent_id].nodes[sdfg_elem.scope_entry];
      } else sdfg_elem = null;
        ...
        if (foreground_elem) {
          if (event.ctrlKey) {
            ...
            if (this.selected_elements.includes(foreground_elem)) {
              ...
              this.selected_elements = this.selected_elements.filter((el) => {
    ...
    if (this.external_mouse_handler)
      dirty |= this.external_mouse_handler(
        evtype,
        event,
        { x: comp_x_func(event), y: comp_y_func(event) },
        elements,
        this,
        this.selected_elements,
        ends_drag
        ...
        if (vscode) {
          function clean_selected(selected_elements) {
            ...
            selected_elements.forEach((el) => {
              let parent_id = el.parent_id === null ? -1 : el.parent_id;
              let type = "other";
              if (el.data.node) type = "node";
              else if (el.data.state) type = "state";
              else if (el.data.type === "InterstateEdge") type = "isedge";
              else if (el.data.type === "Memlet") type = "edge";
              elems.push({
                type: type,
